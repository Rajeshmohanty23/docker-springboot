...
NOTE: There is also a Getting Started Guide on https://github.com/hardik402/docker/blob/main/README.md[Docker], which would also be a good starting point, but it doesn't cover the range of choices that we have here, or in as much detail.
...
```
docker run -network host -p 8085:8085 -d dockersb
```
...
```
$ docker run --name dockersb -ti --entrypoint /bin/sh dockersb
