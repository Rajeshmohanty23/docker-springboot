...
== Introduction to Docker Labs Platform

1. Login with your docker ID and password.
2. Visit Docker Lab here : https://labs.play-with-docker.com/[Docker Labs]
3. Create a folder named `funtainers` (Optional)
4. Git clone the https://github.com/Rajeshmohanty23/docker-springboot[Git Repository]
[Source]
----
mkdir funtainers
git clone https://github.com/Rajeshmohanty23/docker-springboot
cd docker-springboot
----

5. Create a docker file in order to create a docker image.
...
```
$ docker build --build-arg JAR_FILE=target/*.jar -t dockersb .
```
...
```
$ docker run --name myapp -ti --entrypoint /bin/sh dockersb
$ docker exec -ti myapp /bin/sh
/ #
```

where `dockersb` is the `--name` passed to the `docker run` command. If you didn't use `--name` then docker assigns a mnemonic name which you can scrape from the output of `docker ps`. You could also use the SHA identifier of the container instead of the name, also visible from `docker ps`.
